...
class KRAKENQFLHISTORY():
    ...
    def get_ohlc_calculations(self, df, fitted_params=None):
        ...
        if self.trading_style == 'QFL':
            _bid_baseline = df['Low Line'].iloc[-1]
            _ask_baseline = df['High Line'].iloc[-1]
        elif self.trading_style == 'Account Building':
...
class KRAKENQFLBOT(ScriptStrategyBase):
    ...
    trading_style: str = 'QFL'):
        ...
        # Trade size
        self.min_order_size_bid =  Decimal(0.0)
        self.min_order_size_ask =  Decimal(0.0)
        ...
        ## Trading Fee per single trade
        self.fee_percent = 1 / 4 / 100  # Convert percentage to a fraction method 0.0025
        self.total_spent = 0
        self.total_bought = 0
        self.total_earned = 0
        self.total_sold = 0
        ...
        print(f"Initialized KRAKENQFLBOT with trading pair: {self.trading_pair}, exchange: {self.exchange}, style: {self.trading_style}")
        print(f"Min profitability: {self.min_profitability}, Buy P: {self.buy_p}, Sell P: {self.sell_p}")
        print(f"Order amount: {self.order_amount}, Max order amount: {self.max_order_amount}")
    ...
    def determine_log_multipliers(self):
        ...
        maximum_bp = self.buy_p
        maximum_sp = self.sell_p
    ...
    def optimal_bid_ask_spread(self):
        ...
        maximum_volatility = Decimal(np.maximum(self.max_vola , self.min_profitability))
