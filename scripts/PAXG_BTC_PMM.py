...
class SimplePMM(ScriptStrategyBase):
    ...
    def determine_entry_placement(self, max_levels=5):
        ...
        def create_order_levels(is_buy_data, is_sell_data, new_trade_cycle, max_levels):
            max_quote_imbalance = max(quote_balancing_volume, max_order_size )
            max_base_imbalance = max(base_balancing_volume, max_order_size )
            # Depending on the cycle, calculate order sizes
            if (not is_buy_data and not is_sell_data) or (new_trade_cycle):
                bid_order_levels, bid_max_full_orders = calculate_dynamic_order_sizes(quote_balancing_volume, self.min_order_size_bid, max_quote_imbalance, max_levels)
                ask_order_levels, ask_max_full_orders = calculate_dynamic_order_sizes(base_balancing_volume, self.min_order_size_ask, max_base_imbalance, max_levels)
            ...
            # Mid trade logic
                ...
                if is_buy_net: 
                    ...
                    bid_order_levels, bid_max_full_orders = calculate_dynamic_order_sizes(quote_balancing_volume, self.min_order_size_bid, quote_balancing_volume, max_levels)
                    ask_order_levels, ask_max_full_orders = calculate_dynamic_order_sizes(base_balancing_volume, self.min_order_size_ask, max_base_imbalance, max_levels)
